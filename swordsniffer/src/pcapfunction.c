...
//需要root 权限
	...
	} else 
	{
		...
		{
			if (strstr(alldevsp->name, "eth") > 0|| strstr(alldevsp->name, "wlan") > 0) 
			{
...
void
tcp_callback(u_char * arg, const struct pcap_pkthdr *pkthdr,
	     const u_char * packet)
{
	tcpn++;
...
void
udp_callback(u_char * arg, const struct pcap_pkthdr *pkthdr,
	     const u_char * packet)
{
	udpn++;
	...
	sprintf(spacket.summary, "DstPort=%u, SrcPort=%u, Datagram Length=%u", dst_port,src_port,len);
}

void
icmp_callback(u_char * arg, const struct pcap_pkthdr *pkthdr,
	      const u_char * packet)
{
	icmpn ++;
...
void
ip_callback(u_char * arg, const struct pcap_pkthdr *pkthdr,
	    const u_char * packet)
{	
	ipn ++;
...
void
arp_callback(u_char * arg, const struct pcap_pkthdr *pkthdr,
	     const u_char * packet)
{
	arpn ++;
	...
	if(1 == oper)
	{
		sprintf(spacket.summary,"who has %s ? tell %s",spacket.dst_ip, spacket.src_ip);
	}
	else if(2 == oper)
	{
		sprintf(spacket.summary,"%s 's MAC Address is %s",spacket.src_ip, src_mac);
	}
	       ...
	       const u_char * packet)
{
	...
	{
		//dump to a cap file
		pcap_dump(pcapfp, pkthdr, packet);
...
//sleep(0.5);
	...
	GtkWidget *overall_view = GTK_WIDGET(gtk_builder_get_object(builder, "treeview_overall"));
	GtkTreeModel *model = gtk_tree_view_get_model(GTK_TREE_VIEW(overall_view));
	GtkListStore *store = GTK_LIST_STORE(model);
	GtkTreeIter    iter;
	gtk_list_store_append(store, &iter);
	if(gtk_list_store_iter_is_valid(store, &iter))
	{
		gtk_list_store_set(store, &iter, COL_NO, nostr, COL_TIME, time_str,COL_SRC,spacket.src_ip,COL_DST,spacket.dst_ip, COL_PRO,spacket.protocol, COL_LEN, spacket.len, COL_SUMMARY, spacket.summary,-1); 
	}
}
